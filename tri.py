...
class TRI():
    ...
    optional_configs_names = ["load_saved_pretreatment", "add_non_saved_anonymizations",
        ...
        "use_noisy_background_knowledge", "only_use_noisy_background_knowledge",
        "use_mask_background_knowledge", "only_use_mask_background_knowledge",  # 👈 新增这两个
    ...
    use_noisy_background_knowledge = False  # 👈 新增
    only_use_noisy_background_knowledge = False  # 👈 新增
    use_mask_background_knowledge = False  # 👈 新增
    only_use_mask_background_knowledge = False  # 👈 新增
    ...
    def run_data(self, verbose=True):
        ...
        # 👇 新增：保存训练时的 label 顺序映射，方便后续 distance 侧严格对齐
        # 同时也用于后续 PIInum/PL 分析脚本把 label 准确映射回 Name，避免错位。
        try:
            mapping_path = os.path.join(self.output_folder_path, "label_mapping.json")
                ...
                json.dump(
                    {
                        ...
                        "rule": "sorted(unique(individual_name))",
                        "individual_name_column": self.individual_name_column,
                    },
                    f,
                    ensure_ascii=False,
                    indent=2,
            ...
            if verbose:
                logging.info(f"Saved label mapping: {mapping_path}")
        except Exception as e:
            logging.warning(f"Failed to save label mapping: {e}")
        ...
        if (self.anonymize_background_knowledge or self.use_document_curation or 
            self.use_noisy_background_knowledge or self.use_mask_background_knowledge) and not self.pretreated_data_loaded:
            if verbose: logging.info("######### START: DATA PRETREATMENT #########")
            
            # 👇 新增：Mask BK处理
            if self.use_mask_background_knowledge:
                if verbose: logging.info("######### START: ADDING MASK BACKGROUND KNOWLEDGE #########")        
                self.train_df = self.add_mask_bk(self.train_df, self.data_df)
                if verbose: logging.info("######### END: ADDING MASK BACKGROUND KNOWLEDGE #########")
            else:
                if verbose: logging.info("######### SKIPPING: MASK BACKGROUND KNOWLEDGE #########")
            
            # 👇 噪声BK处理
            if self.use_noisy_background_knowledge:
                if verbose: logging.info("######### START: ADDING NOISY BACKGROUND KNOWLEDGE #########")        
                self.train_df = self.add_noisy_bk(self.train_df, self.data_df)
                if verbose: logging.info("######### END: ADDING NOISY BACKGROUND KNOWLEDGE #########")
            else:
                if verbose: logging.info("######### SKIPPING: NOISY BACKGROUND KNOWLEDGE #########")
    ...
    def read_data(self) -> pd.DataFrame:
        ...
        if self.dev_set_column_name is not False and not self.dev_set_column_name in data_df.columns:
            raise Exception(f"Dataframe does not contain the dev set column {self.dev_set_column_name}")
                ...
                # 👇 新增：检查噪声列是否存在
        if self.use_noisy_background_knowledge and "noise_public_knowledge" not in data_df.columns:
            logging.exception(f"use_noisy_background_knowledge is True but 'noise_public_knowledge' column not found in data")
        
        # 👇 新增：检查mask列是否存在
        if self.use_mask_background_knowledge and "replaced_background" not in data_df.columns:
            logging.exception(f"use_mask_background_knowledge is True but 'replaced_background' column not found in data")
        ...
        data_df = data_df.sort_values(self.individual_name_column).reset_index(drop=True)

        if all(col in data_df.columns for col in ['Original', 'mask_abstract','replaced_abstract','noise_0p001_abstract', 'noise_1p0_abstract', 'noise_2p0_abstract', 'noise_3p0_abstract', 'noise_4p0_abstract', 'noise_5p0_abstract', 'noise_6p0_abstract', 'noise_7p0_abstract', 'noise_8p0_abstract', 'noise_9p0_abstract', 'noise_10p0_abstract']):
            logging.info("✓ 所有12个eval列都存在")
        return data_df

    def split_data(self, data_df:pd.DataFrame):
        # Avoid mutating the caller's dataframe (important when loading saved pretreatment)
        ...
        eval_columns = ['Original', 'mask_abstract','replaced_abstract','noise_0p001_abstract', 'noise_1p0_abstract', 'noise_2p0_abstract', 'noise_3p0_abstract', 'noise_4p0_abstract', 'noise_5p0_abstract', 'noise_6p0_abstract', 'noise_7p0_abstract', 'noise_8p0_abstract', 'noise_9p0_abstract', 'noise_10p0_abstract' ]
        
        # 创建eval_dfs，只包含指定列（不要写 self.eval_dfs，避免覆盖已加载的 pretreatment）
        eval_dfs = OrderedDict()
        for col in eval_columns:
            if col in data_df.columns:
                eval_df = data_df[[self.individual_name_column, col]].copy()
                eval_df.columns = [self.individual_name_column, 'protected_document']
                eval_dfs[col] = eval_df
            else:
    ...
    # 👇 新增方法
    def add_noisy_bk(self, train_df:pd.DataFrame, data_df:pd.DataFrame) -> pd.DataFrame:
        """
        使用 noise_public_knowledge 列作为训练数据
        从原始 data_df 中提取 noise_public_knowledge 列并拼接到训练集
        
        注意：当与 mask_bk 配合使用时：
        - 如果 only_use_mask_background_knowledge=true，train_df 已经是 mask 数据
        - 此时 only_use_noisy_background_knowledge=false 会将 noise 拼接到 mask 数据
        - 最终结果：mask + noise（无原始数据）
        """
        noise_cols = [self.individual_name_column, "noise_public_knowledge"]
        
        train_noise_df = data_df[noise_cols].dropna().reset_index(drop=True)
        
        # 重命名列以匹配 train_df 的列名
        train_noise_df.columns = train_df.columns
        
        if self.only_use_noisy_background_knowledge:
            # 完全替换当前数据，只使用 noise 数据
            train_df = train_noise_df
        else:
            # 拼接到当前数据（可能是原始数据、mask数据或其组合）
            train_df = pd.concat([train_df, train_noise_df], ignore_index=True, copy=False)
        
        return train_df

    # 👇 修改方法：更清晰的逻辑
    def add_mask_bk(self, train_df:pd.DataFrame, data_df:pd.DataFrame) -> pd.DataFrame:
        """
        使用 replaced_background 列作为训练数据
        从原始 data_df 中提取 replaced_background 列并拼接到训练集
        """
        mask_cols = [self.individual_name_column, "replaced_background"]
        
        train_mask_df = data_df[mask_cols].dropna().reset_index(drop=True)
        
        # 重命名列以匹配 train_df 的列名
        train_mask_df.columns = train_df.columns
        
        if self.only_use_mask_background_knowledge:
            # 完全替换原始数据，只使用 mask 数据作为基础
            train_df = train_mask_df
        else:
            # 拼接到原始数据
            train_df = pd.concat([train_df, train_mask_df], ignore_index=True, copy=False)
    ...
    def run_build_classifier(self, verbose=True):
        ...
        # Otherwise, pretrain (if required) and finetune a TRI model
            ...
            if self.save_finetuning:
                ...
                self.pipe, self.tri_model = self.save_finetuned_tri_pipeline(self.finetuning_trainer, self.tokenizer)
    ...
    def save_finetuned_tri_pipeline(self, finetuning_trainer, tokenizer):
        # IMPORTANT: always save the model currently held by Trainer (best model if load_best_model_at_end=True)
        model = finetuning_trainer.model
        if hasattr(model, "module"):  # DDP / DataParallel / some accelerate wrappers
            model = model.module

        # Save model+tokenizer in a way that AutoModelForSequenceClassification.from_pretrained can reload
        model.save_pretrained(self.tri_pipe_path)
        tokenizer.save_pretrained(self.tri_pipe_path)

        # Keep an in-memory pipeline/model for later use
    ...
    def get_trainer(self, model, task_config, train_dataset, eval_datasets_dict=None, data_collator=None):
        ...
        if is_for_mlm:
            ...
            save_class_embeddings = False  # 👈 预训练时不保存
        ...
        # Settings for finetuning
            ...
            save_class_embeddings = True  # 👈 微调时保存
        ...
        args = TrainingArguments(
            ...
            metric_for_best_model='eval_avg_Accuracy',  # 改为平均准确率
        ...
        trainer = TRITrainer(results_filepath,
                            model=model,
                            args=args,
                            save_class_embeddings=save_class_embeddings,
                            output_folder_path=self.output_folder_path,  # 👈 新增
...
# 全局变量用于在 compute_metrics 中访问
current_dataset_name = None
current_output_folder_path = None  # 👈 新增

class TRITrainer(Trainer):
    def __init__(self, results_filepath:str = None, save_class_embeddings:bool = True, output_folder_path:str = None, **kwargs):  # 👈 添加参数
        Trainer.__init__(self, **kwargs)
        self.results_filepath = results_filepath
        self.save_class_embeddings = save_class_embeddings
        self.output_folder_path = output_folder_path  # 👈 新增
        ...
        if self.do_custom_eval:
            ...
            self.initialize_results_file()
                # 新增：保存类别嵌入距离矩阵（只保存一次）
            if self.save_class_embeddings:
                self.extract_and_save_class_embeddings()
    
    def current_time_str(self):
        return datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    def extract_and_save_class_embeddings(self):
        """提取并保存类别嵌入距离矩阵"""
        try:
            import numpy as np
            ...
            logging.info("提取类别嵌入距离矩阵...")
            
            # 从分类器提取类别嵌入
            model = self.model
            if hasattr(model, 'module'):  # 处理 DataParallel/DistributedDataParallel
                model = model.module
            ...
            if hasattr(model, 'classifier'):
                if hasattr(model.classifier, 'weight'):
                    ...
                    logging.warning("无法找到分类器权重，跳过类别嵌入保存")
                    return
            else:
                logging.warning("模型没有 classifier 属性，跳过类别嵌入保存")
                return
            
            # 计算余弦距离矩阵
            distance_matrix = cosine_distances(class_embeddings)
            np.fill_diagonal(distance_matrix, 1e-12)  # 避免除零
            
            # 👇 修改：使用 output_folder_path 构建文件路径
            if self.output_folder_path:
                output_path = os.path.join(self.output_folder_path, "class_distance_matrix.json")
            else:
            ...
            logging.info(f"✅ 类别嵌入距离矩阵已保存到: {output_path}")
            logging.info(f"   形状: {distance_matrix.shape}")
            logging.info(f"   距离统计: 均值={distance_matrix.mean():.4f}, "
                        f"标准差={distance_matrix.std():.4f}, "
                        f"最小={distance_matrix.min():.4f}, "
                        f"最大={distance_matrix.max():.4f}")
            
        except Exception as e:
            logging.warning(f"保存类别嵌入距离矩阵失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    # 修改TRITrainer.evaluate方法：
    def evaluate(self, eval_dataset=None, ignore_keys=None, metric_key_prefix="eval"):
        if self.do_custom_eval:
            global current_dataset_name  # Access global variable
            metrics = OrderedDict()
            ...
            for dataset_name, dataset in self.eval_datasets_dict.items():
                current_dataset_name = dataset_name  # Set global variable before evaluation
                dataset_metrics = Trainer.evaluate(self, eval_dataset=dataset, ignore_keys=ignore_keys, metric_key_prefix=metric_key_prefix)  # Remove compute_metrics parameter
...
def compute_metrics(results):
    global current_dataset_name, current_output_folder_path  # 👈 修改
    logits, labels = results

    # 保存每个 sample 的 softmax 概率分布（按 dataset 输出到文件）
    # logits 形状为 (n_samples, num_classes)
    probs = F.softmax(torch.from_numpy(np.asarray(logits)), dim=-1).cpu().numpy()
    ...
    out = {
        ...
        "pred_idxs": probs.argmax(axis=1).tolist()
    }

    import json
    # 👇 修改：使用 output_folder_path 构建文件路径
    if current_output_folder_path:
        filename = os.path.join(current_output_folder_path, f"eval_results_{current_dataset_name}.json")
    else:
        filename = f"eval_results_{current_dataset_name}.json"
    
    with open(filename, "w") as f:
        json.dump(out, f)

    # 保持原有按 label 累加 logits 并计算最终 Accuracy 的逻辑（用于兼容现有流程）
    logits_t = torch.from_numpy(np.asarray(logits))
    logits_dict = {}
    for logit, label in zip(logits_t, labels_np):
        label = int(label)
    ...
    for idx, (label, logits_sum) in enumerate(logits_dict.items()):
        all_labels[idx] = label
        probs_label = F.softmax(logits_sum, dim=-1)
        all_preds[idx] = torch.argmax(probs_label)
